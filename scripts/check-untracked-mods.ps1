<#
  ...
  commit. In THIS repo it was `393c38c "chore: commit idle working-tree changes"`, which staged
  every MODIFIED tracked file and none of the NEW untracked ones, so `main` declared `mod nudge;`
  and `mod nudge_engine;` while neither file was ever added. CI went red and stayed red until
  somebody happened to look. SageThumbs 2K's `02d730a`, same commit title, same day, same defect.
  ...
  the whole fleet (`--all`), and SageThumbs 2K carries the same gate. This copy is the one that
  runs here; keep the three in step if the resolution rules ever need to change.
