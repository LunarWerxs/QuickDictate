//! The Advanced page: the switches that are set once and then forgotten
//! (key prewarm, the tray icon, logging, error reports, usage stats), the
//! data folder, and the Per-App Profiles editor. They used to share the
//! Application page with the everyday toggles, and that page had grown into
...
impl super::SettingsApp {
    pub(crate) fn advanced_card(&mut self, ui: &mut egui::Ui) {
        card(ui, |ui| {
    ...
    /// The set-and-forget toggles, split across two columns: the app-level
    /// ones on the left, the diagnostics and data-sharing ones on the right.
    fn advanced_toggles(&mut self, ui: &mut egui::Ui) {
        ui.columns(2, |cols| {
            let left = &mut cols[0];
            blue_check(
                left,
    ...
    pub(crate) fn data_folder_section(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ui.label(subsection_title("Files"));
            ui.label(
    ...
    /// `advanced_toggles`); this is the action + preview that only shows
    ...
    fn error_report_section(&mut self, ui: &mut egui::Ui) {
        ...
        subsection_start(ui);
        ui.horizontal(|ui| {
            ui.label(subsection_title("Error reporting"));
    ...
    fn active_profiles_section(&mut self, ui: &mut egui::Ui) {
        ...
        subsection_start(ui);
        ui.label(subsection_title("Active profiles"));
        ui.label(
            RichText::new(
