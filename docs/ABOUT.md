...
- Dictation history lives in memory only (capped at 50 entries) and is explicitly not persisted to disk, so it resets on every restart - a known, stated gap rather than an oversight. anchors: `src/state.rs:17`
...
- **Custom vocabulary** - A user-maintained word list is sent to the active provider as recognition hints (or biasing terms) so jargon and names are transcribed correctly, and can be overridden per profile. - `src/config/schema.rs:402`, `src/stt/provider.rs:74`, `src/config/schema.rs:74`
...
- **Searchable dictation history** - The last dictations are kept in memory and browsable/filterable in Settings, with one-click copy of any past transcript. - `src/state.rs:33`, `src/settings_ui/history_sync.rs:13`, `src/settings_ui/text.rs:66`
...
- **Settings window** - A single egui window (Application / Dictation / History tabs) covering provider and key setup, hotkeys, formatting, profiles, and history, opened from the tray or on first run. - `src/settings_ui/mod.rs:164`, `src/settings_ui/nav.rs:23`, `src/settings_ui/cards.rs:109`
- **API key pool with health tracking and failover** - Multiple keys can be configured per provider; the pool tracks per-key status (untested/alive/quota/dead), cools down failing keys, and automatically rotates to the next usable one. - `src/keys.rs:81`, `src/keys.rs:216`, `src/keys.rs:19`
...
- **Optional settings sync via Connections sign-in** - Signing in with a LunarWerx Connections account (OAuth/PKCE) syncs preferences like hotkeys and language across machines and merges usage stats; API keys, audio, and transcripts are never synced. - `src/sync/oauth.rs:151`, `src/sync/mod.rs:118`, `src/settings_ui/history_sync.rs:115`
- **Silent self-update** - Checks GitHub releases on a timer, verifies the new executable's hash, and can download and swap the running portable exe in place with the app relaunching itself; asks before installing. - `src/update/mod.rs:235`, `src/update/install.rs:164`, `src/update/flows.rs:47`
...
_Generated by `odin codex about --publish quickdictate-beta1` on 2026-09-09 from a Codex dossier stamped 2026-09-05. Regenerate after the product moves; `odin codex about` reports drift._
<!-- odin:about GENERATED END sha=1c3ea40f92e7 -->
